// SemperTests/ISO226ContoursTests.swift
...
struct LoudnessCompensatorHeadroomTests {
    ...
    @Test("Low-volume compensation reserves digital headroom")
    func lowVolumeCompensationUsesPreamp() {
        let sampleRate = 48_000.0
        let phon = ISO226Contours.estimatedPhon(fromSystemVolume: 0.1)
        let gains = LoudnessCompensator.fittedSectionGains(forPhon: phon, sampleRate: sampleRate)
        let preamp = LoudnessCompensator.requiredPreampGain(
            sectionGains: gains,
            sampleRate: sampleRate
        )
        ...
        processor.updateForVolume(0.1)
        #expect(abs(processor.preampGain - preamp) < 0.000_001)
    }

    @Test("Reference volume keeps unity preamp")
    func referenceVolumeUsesUnityPreamp() {
        let processor = LoudnessCompensator(sampleRate: 48_000)
        processor.updateForVolume(1)

        #expect(processor.preampGain == 1)
        #expect(processor.isEnabled == false)
    }
