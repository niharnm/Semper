// SemperTests/SoftLimiterAdversarialTests.swift
...
struct SoftLimiterCeilingTests {
    ...
    @Test("Output never exceeds +ceiling for positive inputs",
          arguments: [Float(0.9), 1.0, 1.5, 2.0, 5.0, 10.0, 100.0, 1000.0, 1e10, 1e20,
    ...
    @Test("Output never exceeds -ceiling for negative inputs",
          arguments: [Float(-0.9), -1.0, -1.5, -5.0, -100.0, -1e20,
                      -Float.greatestFiniteMagnitude])
    ...
    @Test("Passthrough is bit-exact at and below threshold",
          arguments: [Float(0.0), 0.001, 0.1, 0.5, 0.8, 0.88, 0.89,
                      Float(-0.5), -0.8, -0.89])
    ...
    @Test("Output strictly below ceiling for moderate inputs above threshold",
          arguments: [Float(0.9), 1.0, 2.0, 10.0, 100.0, 1000.0])
...
struct SoftLimiterPropertiesTests {
    ...
    func monotonicityPositive() {
        let inputs: [Float] = [0.0, 0.5, 0.88, SoftLimiter.threshold, 0.892, 0.9, 1.0, 1.5, 2.0, 5.0, 10.0, 100.0]
    ...
    func monotonicityNegative() {
        let inputs: [Float] = [-100.0, -10.0, -2.0, -1.0, -0.9, -SoftLimiter.threshold, -0.5, 0.0]
        for i in 0..<(inputs.count - 1) {
            let out1 = SoftLimiter.apply(inputs[i])
            let out2 = SoftLimiter.apply(inputs[i + 1])
            #expect(out2 >= out1,
    ...
    func continuityAtThreshold() {
        ...
        #expect(delta < 1e-4,
                "Discontinuity at threshold: f(t)=\(atThreshold), f(t+1e-6)=\(justAbove), delta=\(delta)")
    }
    ...
    func decreasingDerivative() {
        ...
        let inputs: [Float] = [0.9, 1.0, 1.5, 2.0, 5.0]
        let dx: Float = 0.01
...
struct SoftLimiterEdgeTests {
    ...
    func constantsConsistent() {
        #expect(abs(SoftLimiter.threshold - 0.891_250_9) < 1e-7)
        #expect(SoftLimiter.ceiling == 1.0)
        #expect(SoftLimiter.headroom == SoftLimiter.ceiling - SoftLimiter.threshold)
        #expect(abs(SoftLimiter.headroom - 0.108_749_1) < 1e-6,
                "Headroom should be about 0.10875, got \(SoftLimiter.headroom)")
    }
}
...
struct SoftLimiterBufferTests {
    ...
    func bufferBelowThreshold() {
        ...
        let values: [Float] = [0.1, -0.1, 0.5, -0.5, 0.8, -0.8, 0.89, -0.89]
    ...
    func bufferExactlyAtThreshold() {
        ...
        // Peak equals the threshold, so the fast path returns.
