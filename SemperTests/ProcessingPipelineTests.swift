// SemperTests/ProcessingPipelineTests.swift
...
struct StereoBalanceTests {
    @Test("Full-left balance silences only the right channel")
    func fullLeft() {
        let frames = 32
        ...
        for frame in 0..<frames {
            #expect(samples[frame * 2] == 0.5)
            #expect(samples[frame * 2 + 1] == 0)
        }
    }

    @Test("Center reset restores equal channel gain")
    func centerReset() {
        let frames = 32
        let input = TestABL(buffers: [(channels: 2, frames: frames)])
        let output = TestABL(buffers: [(channels: 2, frames: frames)])
        ...
        for frame in 0..<frames {
            #expect(samples[frame * 2] == 0.5)
            #expect(samples[frame * 2 + 1] == 0.5)
        }
    }
}

@Suite("ProcessTapController Stereo Metering")
struct StereoMeteringTests {
    @Test("Reports distinct processed left and right peaks")
    func distinctStereoPeaks() {
        let frames = 32
        let input = TestABL(buffers: [(channels: 2, frames: frames)])
        let output = TestABL(buffers: [(channels: 2, frames: frames)])
        ...
        var volume: Float = 1
        processWithDefaults(input: input, output: output, currentVol: &volume)

        let level = ProcessTapController.stereoPeaks(
            in: output.bufferList,
            preferredStereoLeft: 0,
            preferredStereoRight: 1
        )
        #expect(abs(level.left - 0.6) < 0.0001)
        #expect(abs(level.right - 0.25) < 0.0001)
    }

    @Test("Meter includes the applied balance")
    func balanceChangesMeasuredChannels() {
        let frames = 32
        let input = TestABL(buffers: [(channels: 2, frames: frames)])
        let output = TestABL(buffers: [(channels: 2, frames: frames)])
        fill(input, bufferIndex: 0, value: 0.5)
        ...
        #expect(abs(level.left - 0.5) < 0.0001)
        #expect(level.right == 0)
    }

    @Test("Mono output is reported on both channels")
    func monoDuplicatesPeak() {
        let output = TestABL(buffers: [(channels: 1, frames: 8)])
        fill(output, bufferIndex: 0, value: 0.4)
        ...
        #expect(abs(level.left - 0.4) < 0.0001)
        #expect(abs(level.right - 0.4) < 0.0001)
    }
}

@Suite("ProcessTapController Device Switch Selection")
...
struct LoudnessIntegrationTests {
    ...
    func loudnessCompensatorModifiesOutput() {
        ...
        // Create a stereo sine wave input at 60 Hz. Loudness compensation changes
        // the low-frequency balance while its preamp preserves digital headroom.
        ...
        // Compensation and its headroom preamp should change the output.
