...
If cleanup fails, Shelf retains the affected items and file access. Use **Retry Cleanup** in the image options or the File Shelf recovery view. Recovery remains available after a failed Pause or Quit. An affected source waits for that retry before expiring; other shelf items can still expire. If a copy was created but its location or cleanup cannot be verified, Shelf says so and blocks another save. Retry recovers the existing copy without creating another one. Any paths shown during recovery are locations to check, not a confirmed saved-file receipt.
