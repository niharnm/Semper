...
Semper 可以使用来自 [AutoEQ](https://github.com/jaakkopasanen/AutoEq) 项目的耳机专属频响校正配置。它会针对你耳机本身的频率曲线做补偿，让声音更平直、更准确。

## 工作原理

每副耳机对声音的染色都不一样：有的低频偏多，有的高频刺耳。AutoEQ 测量出这些偏离，并生成对应的修正 EQ 滤波器。Semper 会按设备分别应用这些滤波器，所以每副耳机都拥有自己独立的校正配置。

校正会叠加在 Semper 的 10 段 EQ 之上，因此应用一份配置之后，你仍然可以按个人口味继续微调。

## 浏览内置配置

1. 点击 Semper 中任一耳机设备旁的 **魔棒图标**
2. 按型号搜索你的耳机
3. 选中一个配置，立即生效
...
Semper 接受 [EqualizerAPO](https://sourceforge.net/projects/equalizerapo/) 的 ParametricEQ.txt 文件：
...
- **内置搜索** —— 最方便的方式。Semper 直接内置了上千款耳机
- **[autoeq.app](https://www.autoeq.app/)** —— 网页版工具，选项更丰富。把 equalizer app 选成 **EqualizerAPO ParametricEq**，下载文件后导入 Semper 即可
