...
| Safe Eject | `Semper/Storage` | `SafeEjectView` | `storage.open` |
...
before execution. `SafeEjectModule.handle(.open, openDetail: openStorage)` only
navigates. Physical eject remains inside `SafeEjectView` with a selected volume
and confirmation. This stack does not register `storage.ejectAllEligible`.
...
- With an explicitly approved signed build, exercise first-use Accessibility,
  ...
  app minimum sizes, subsequent manual moves, and undo.
