// Semper/Settings/SettingsManager.swift
import Foundation
import os
...
final class SettingsManager {
    ...
    struct Settings: Codable {
        ...
        init(from decoder: Decoder) throws {
            ...
            softwareDeviceSavedVolumes = (try c.decodeIfPresent([String: Float].self, forKey: .softwareDeviceSavedVolumes) ?? [:])
                .filter { $0.value.isFinite && $0.value >= 0 }
                .mapValues { min($0, 1.0) }
            outputMasterGains = (try c.decodeIfPresent([String: Float].self, forKey: .outputMasterGains) ?? [:])
                .filter { $0.value.isFinite && $0.value > 1 }
                .mapValues { min($0, VolumeMapping.maximumMasterGain) }
    ...
    func hasSavedAudioConfiguration(for identifier: String) -> Bool {
        settings.appVolumes[identifier] != nil
            || settings.appDeviceRouting[identifier] != nil
            || settings.appMutes[identifier] != nil
            || settings.appBoosts[identifier] != nil
    ...
    func ignoreApp(_ identifier: String, info: IgnoredAppInfo) {
        ...
        settings.pinnedApps.remove(identifier)
        settings.pinnedAppInfo.removeValue(forKey: identifier)
        // Clear per-app settings — Semper won't interact with this app
    ...
    // MARK: - Master Output Processing

    func getOutputMasterGain(for deviceUID: String) -> Float? {
        settings.outputMasterGains[deviceUID]
    }

    func setOutputMasterGain(for deviceUID: String, to gain: Float?) {
        if let gain, gain.isFinite, gain > 1 {
            settings.outputMasterGains[deviceUID] = min(gain, VolumeMapping.maximumMasterGain)
        } else {
            settings.outputMasterGains.removeValue(forKey: deviceUID)
        }
        scheduleSave()
    }

    func getOutputBalance(for deviceUID: String) -> Float? {
        settings.outputBalances[deviceUID]
    }

    func setOutputBalance(for deviceUID: String, to balance: Float?) {
        if let balance, balance.isFinite, abs(balance) >= 0.0001 {
    ...
    func updateAppSettings(_ newSettings: AppSettings) {
        ...
        if managesLaunchAtLogin,
           newSettings.launchAtLogin != settings.appSettings.launchAtLogin {
