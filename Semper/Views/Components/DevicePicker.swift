// Semper/Views/Components/DevicePicker.swift
import SwiftUI
...
extension DevicePicker {
    static func routingSubtitle(
        ...
        lifecycle: AppRouteLifecycle? = nil
    ) -> String? {
        func deviceName(for uid: String) -> String {
            devices.first(where: { $0.uid == uid })?.name ?? "Unknown Output"
        }

        if let lifecycle {
            switch lifecycle {
            case .preparing(let deviceUIDs):
                let destination = deviceUIDs.map(deviceName).joined(separator: " + ")
                return "Routing to \(destination)…"
            case .active(let deviceUIDs):
                if deviceUIDs.count > 1 {
                    return deviceUIDs.map(deviceName).joined(separator: " + ")
                }
                if let uid = deviceUIDs.first {
                    return isFollowingDefault
                        ? "System Output · \(deviceName(for: uid))"
                        : "\(deviceName(for: uid)) · Routed"
                }
            case .failed(let previousDeviceUIDs, _):
                if let previousUID = previousDeviceUIDs.first {
                    return "Route failed · Still on \(deviceName(for: previousUID))"
                }
                return "Route failed"
        ...
        case .single:
            if isFollowingDefault {
                if let defaultDeviceUID {
                    return "System Output · \(deviceName(for: defaultDeviceUID))"
                }
