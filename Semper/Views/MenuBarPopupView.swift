// Semper/Views/MenuBarPopupView.swift
import AudioToolbox
import SwiftUI
...
struct MenuBarPopupView: View {
    ...
    /// Hover state for the website link.
    ...
    private var outputDeviceMenu: some View {
        Menu {
            ...
            Button("Show output controls", systemImage: "speaker.wave.2.fill") {
                showingInputDevices = false
            }
        } label: {
            popupDevicePill(
                title: defaultOutputDeviceName,
    ...
    private var inputDeviceMenu: some View {
        Menu {
            ...
            Button("Show input controls", systemImage: "mic.fill") {
                showingInputDevices = true
            }
        } label: {
            popupDevicePill(
                title: defaultInputDeviceName,
    ...
    private var editPriorityButton: some View {
        Button {
            ...
            Image(systemName: isEditingDevicePriority ? "checkmark" : "chart.bar.xaxis")
                .font(.system(size: 12, weight: isEditingDevicePriority ? .bold : .regular))
                .foregroundStyle(
                    isEditingDevicePriority
                        ? Color.white
    ...
    private var popupFooter: some View {
        HStack {
            Button {
                NSWorkspace.shared.open(DesignTokens.Links.website)
            } label: {
                HStack(spacing: 5) {
            ...
            .accessibilityLabel("Open Semper website")
        ...
        .padding(.horizontal, 12)
        .padding(.vertical, 8)
        .background(DesignTokens.Colors.nextFooterBackground)
    ...
    private var visibleMasterOutputDevices: [AudioDevice] {
        if let defaultDevice = sortedDevices.first(
            where: { $0.id == deviceVolumeMonitor.defaultDeviceID }
        ) {
            return [defaultDevice]
        }
        return Array(sortedDevices.prefix(1))
    }
            ...
            if isEditingDevicePriority {
                ...
                ForEach(visibleMasterOutputDevices) { device in
                    ...
                    DeviceRow(
                        ...
                        isLevelMeterActive: isPopupVisible,
                        attenuationNotice: audioEngine.outputAttenuationNotice(for: device.uid),
        ...
        VStack(alignment: .leading, spacing: 0) {
            HStack {
                ...
                if !isEditingDevicePriority {
                    ...
                    .foregroundStyle(DesignTokens.Colors.systemGreen)
                    .help("Play a short output test")
                }
    ...
    private func inactiveAppRow(info: PinnedAppInfo, displayableApp: DisplayableApp, userPresets: [UserEQPreset], scrollProxy: ScrollViewProxy) -> some View {
        ...
        InactiveAppRow(
            appInfo: info,
            icon: displayableApp.icon,
            usesFallbackIcon: displayableApp.usesFallbackIcon,
    ...
    /// Brings the source application and all of its windows forward.
    /// Returns false for headless processes so the row can show identity details.
    private func activateApp(pid: pid_t?, bundleID: String?) -> Bool {
        let activated = AppActivationService.activate(pid: pid, bundleID: bundleID)
        if activated {
            NSApp.keyWindow?.resignKey()
        }
        return activated
