// Semper/Views/HUD/HUDWindowController.swift
import AppKit
import SwiftUI
import os
...
final class HUDWindowController: MediaKeyHUDPresenting {
    ...
    private let logger = Logger(subsystem: "systems.semper.Semper", category: "HUDWindowController")
    ...
    /// Displays the HUD. Skipped while the menu bar popup is visible.
    ...
    func showPerAppNotControlledHUD(displayName: String?, bundleID: String?, icon: NSImage?) {
        let title = displayName?.nilIfEmpty
            ?? bundleID?.nilIfEmpty
            ?? "Semper isn't controlling this app yet"
        presentPerApp(
            icon: icon,
            title: title,
    ...
    /// Presents the app-specific HUD for explicit hotkey actions.
    ...
    private func postPerAppAccessibilityAnnouncement(panel: NSPanel, title: String, content: PerAppHUDContent) {
        ...
        case .notControlled:
            description = "\(title), not controlled by Semper"
        }
        NSAccessibility.post(
            element: panel,
            notification: .announcementRequested,
...
private struct PerAppHUD: View {
    ...
    private var subtitleText: String? {
        if case .notControlled = content { return "Not controlled by Semper" }
        return nil
    }
