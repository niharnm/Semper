// Semper/Audio/Extensions/AudioDeviceID+Streams.swift
import AudioToolbox
import Foundation
...
nonisolated extension AudioDeviceID {
    ...
    /// Returns preferred stereo channels as zero-based indices when CoreAudio
    /// provides a valid, distinct pair for the current output format.
    func detectedPreferredStereoChannelIndices() -> (left: Int, right: Int)? {
        var address = AudioObjectPropertyAddress(
        ...
        guard err == noErr, channels.count >= 2 else { return nil }
    ...
    /// Returns preferred stereo channels as zero-based indices, falling back
    /// to the first two channels for existing processing call sites.
    func preferredStereoChannelIndices() -> (left: Int, right: Int) {
        detectedPreferredStereoChannelIndices() ?? (0, 1)
    }
