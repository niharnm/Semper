// Semper/Audio/Monitors/AudioDeviceMonitor.swift
import AppKit
import AudioToolbox
import os
...
final class AudioDeviceMonitor: AudioDeviceProviding {
    ...
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "Semper", category: "AudioDeviceMonitor")
    ...
    private func refresh() {
        do {
            ...
            for deviceID in deviceIDs {
                ...
                // Semper's own internal aggregates (used for process taps) are
                ...
                // etc.) pass through.
                if deviceID.isAggregateDevice() && name.hasPrefix("Semper-") { continue }
                ...
                if deviceID.hasOutputStreams() {
                    ...
                    let device = AudioDevice(
                        ...
                        outputTopology: Self.outputTopology(for: deviceID)
                    )
    ...
    private static func outputTopology(for deviceID: AudioDeviceID) -> OutputDeviceTopology {
        let preferred = deviceID.detectedPreferredStereoChannelIndices().map {
            StereoChannelPair(left: $0.left, right: $0.right)
        }
        return OutputDeviceTopology(
            channelCount: deviceID.outputChannelCount(),
            preferredStereoPair: preferred,
            isAlive: deviceID.isDeviceAlive(),
