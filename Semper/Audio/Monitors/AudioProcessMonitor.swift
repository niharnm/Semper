// Semper/Audio/Monitors/AudioProcessMonitor.swift
import AppKit
import AudioToolbox
import os
...
final class AudioProcessMonitor: AudioProcessMonitoring {
    ...
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "Semper", category: "AudioProcessMonitor")
    ...
    private func refresh() {
        do {
            ...
            var runningProcessIDs: [AudioObjectID] = []
            ...
            for objectID in processIDs {
                ...
                let name = resolvedApp?.localizedName
                    ...
                    ?? "Audio Process \(pid)"
                let resolvedIcon = resolvedApp?.icon
                ...
                if let existing = appsByPID[parentPID] {
                    if !existing.processObjectIDs.contains(objectID) {
                        ...
                        appsByPID[parentPID] = AudioApp(
                            ...
                            usesFallbackIcon: existing.usesFallbackIcon
                        )
                    }
                } else {
                    appsByPID[parentPID] = AudioApp(
                        ...
                        usesFallbackIcon: resolvedIcon == nil
                    )
                }
            }

            // Update per-process listeners
            updateProcessListeners(for: runningProcessIDs)
