...
/// that target curve with a low-cost four-section shelf/bell cascade. A preamp cut
/// preserves headroom before the final limiter catches residual peaks.
///
...
final class LoudnessCompensator: BiquadProcessor, @unchecked Sendable {
    ...
    var preampGain: Float { _preampGain }
    ...
    func updateForVolume(_ systemVolume: Float) {
        ...
        _preampGain = Self.requiredPreampGain(sectionGains: gains, sampleRate: sampleRate)
    ...
    static func requiredPreampGain(sectionGains: [Float], sampleRate: Double) -> Float {
        let peakBoost = max(
            0,
            realizedResponseDB(
                sectionGains: sectionGains.map(Double.init),
                sampleRate: sampleRate
            ).max() ?? 0
        )
        return peakBoost > 0 ? powf(10, Float(-peakBoost) / 20) : 1
    }
